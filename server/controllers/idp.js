...
const Model = require('../models/idp');
...
module.exports = {
  ...
  fetchAll: async (req, res, next) => {
    ...
    const find = await Model.find({}).populate([{path:"competency"}]).skip(skip).limit(10).exec()
      res.json({
        data: find,
  ...
  fetchWithoutPagination: async (req, res, next) => {
    const find = await Model.find({}).populate([{path:"competency"}]).exec()
      res.json({
        data: find       
    })
  },
  fetchSingle: async (req, res, next) => {
    const find = await Model.findOne({_id:req.params.id}).populate([{path:"competency"}]).exec()
    res.json({data: find})
  },
